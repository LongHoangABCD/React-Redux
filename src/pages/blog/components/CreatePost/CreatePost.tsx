...
const CreatePost = () => {
    ...
    useEffect(() => {
        setFormData(edittingPost || initalState)
    }, [edittingPost])

    const dispatch = useDispatch()
    const hanldSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault()
        if (edittingPost != null) {
            dispatch(finishEditingPost(formData))
        } else {
            // const formDataWithId = { ...formData, id: new Date().toISOString() }
            const formDataWithId = { ...formData, id: nanoid() }
    ...
    return (
        <div>
            <form onSubmit={hanldSubmit} onReset={handleCancel}>
                <div className='mb-6'>
